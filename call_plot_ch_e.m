...
E_readout_out = sqrt(2*kappa)*x(:,2) -E_in(t);
%figure(100)
%cla
%plot(t,Eout, t, Eout2)
...
out =sqrt( E_in(time(end)).^2./(2/kappa*trapz(time,E_in(time).^2)+1))%-.14*exp(-(time(end)-2000).^2/((2*110)^2));
else
%length(time)
out = (E_in(time).^2./(2/kappa*cumtrapz(time,E_in(time).^2)+1)).^(.5)%-.14*exp(-(time-2000).^2/((2*110)^2));
%length(out);
%length(E_in(time));
