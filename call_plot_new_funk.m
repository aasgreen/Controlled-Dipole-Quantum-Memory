...
Eout = sqrt(2*kappa)*x(:,2)-E_in2(t);

E_in_tot_sqrd = trapz(t, E_in2(t).^2);
E_readout_out = sqrt(2*kappa)*x(:,2) -E_in2(t);
%figure(100)
%cla
%plot(t,Eout, t, Eout2)
...
%This function contains the equations to be solved.
	...
	%these are the equations to solve
        dy(1) = sqrt(-1)*g_0*dipole2(t)*y(2)-(gamma)*y(1); %sigma

        dy(2) =  -kappa*y(2)+sqrt(2*kappa)*E_in2(t) +g_0*sqrt(-1)*dipole2(t)*y(1);
...
time = (0:time/500:time);
	out =sqrt( E_in(time(end)).^2./(2/kappa*trapz(time,E_in(time).^2)+1))+sqrt(- E_in(time(end)-3000).^2./(2/kappa*trapz(time,E_in(time-3000).^2)+1))%-.14*exp(-(time(end)-2000).^2/((2*110)^2));

else
%length(time)
out = (E_in(time).^2./(2/kappa*cumtrapz(time,E_in(time).^2)+1)).^(.5)+ (-E_in(time-3000).^2./(2/kappa*cumtrapz(time,E_in(time-3000).^2)+1)).^(.5)%-.14*exp(-(time-2000).^2/((2*110)^2));
%length(out);
%length(E_in(time));
...
out = (exp(time/140)-1).*exp(-time/90)*3;
...
%out =zeros(length(time),1) %kluge for read-out
...
Elec_an2 = zeros( [length(t), 1]);
  Eout = -E_in2( t ) + sqrt(2*kappa)*x(:,2);
  Einplt = E_in2( t );
  Dipolplt = dipole2(t);
Ein_tot = trapz(t, E_in2(t).^2);
%for j =1:length(t);
