...
clf
subplot(313)
plot(t,abs(Einplt).^2*10^(3),'--' ,t, abs(Eout_an).^2*10^(3) )
%h2 = legend('E_{in}', 'E_{out}', 'Analytic E_{out}',1)
%set(h2,'fontsize',13);
xlabel('$t$ [$\mu$ s]', 'fontsize',10)
ylabel('$(\text{ns})^{-1}$', 'fontsize',10)
xlim([0,3500])
ylim([0,.45])
text(350,.35,'$|E_\textrm{in}(t)|^2$','FontSize',10)

text(2350,.0035,'$|E_\textrm{out}(t)|^2$','FontSize',10)
text(-300,.0045,'c)')
%
subplot(312)
...
xlim([0,3500])
ylim([0,.11])
text(200,.085,'$g_w(t)$','FontSize',10)
text(2600,.07,'$g_r(t)$','FontSize',10)
text(-300,.12,'b)')
subplot(311) 
...
length(tau_list_w)
chx2=plot(tau_list_w, effective_write.^2, tau_list_r, effect_out_list.^2)
%h12 = legend('Effective Write', 'Effective Read',1)
xlabel('$\tau$', 'fontsize',10)
%ylabel('$ns^{\frac{-1}{2}}$','fontsize',12)
xlim([0,2*3])
ylim([0,2])
text(.6,.4,'|\mathcal{E}_\textrm{in}|^2(\tau)','FontSize',10)
text(4,.8,'|\mathcal{E}_\textrm{out}|^2(\tau)','FontSize',10)
text(-.5,2.6,'a)')
