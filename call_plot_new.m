...
if (time == 0 ) ,
	...
	if time < trans_time; %trans time signifies end of in, begin of out
		...
		t = (trans_time:(time-trans_time)/1000:time);
		out =sqrt(kappa/2*(1-exp(-2*tau_r))*E_in(t(end)-trans_time).^2./(trapz(t,-(1-exp(-2*tau_r))*E_in(t-trans_time).^2)+1));
	%out = 0;
	end
%length(time)
else
	...
	paste = 0;
	out_write = (kappa/2*(1-exp(-2*tau_r))*E_in(t_write-trans_time).^2./(cumtrapz(t_write,-(1-exp(-2*tau_r))*E_in(t_write-trans_time).^2)+1)).^(.5);
...
function out = effec_eout(time);
[kappa, gamma, tau, trans_time, start_time] = constants();
out = sqrt(kappa*(1-quad(Elec_analy2, 0, time)));
end

%this function solves the E without the adiabatic approx
function out = Elec_analy2(time,sigma);
%I don't think we can vecorize, as we are calling sig_aeffec_eouts
...
clf
subplot(312)
plot(t-trans_time,abs(Einplt).^2*10^(3),'--' ,t-trans_time, abs(Eout_an).^2*10^(3) )
%h2 = legend('E_{in}', 'E_{out}', 'Analytic E_{out}',1)
%set(h2,'fontsize',13);
xlabel('$t$ [$n$ s]', 'fontsize',10)
ylabel('$(\textrm{$\mu$s})^{-1}$', 'fontsize',10)
xlim([0-trans_time,3500-trans_time])
ylim([0,4.5])
text(350-trans_time,.35,'$|E_\textrm{in}(t)|^2$','FontSize',10)

text(2350-trans_time,3.5,'$|E_\textrm{out}(t)|^2$','FontSize',10)
text(-300-trans_time,4.5,'b)')
%
subplot(313)
plot(tplt-trans_time,abs( Dipolplt))
%h3 = legend('g(t)')
xlabel('$t$ [ns]', 'fontsize',10)
ylabel('[GHz]', 'fontsize', 10)
xlim([0-trans_time,3500-trans_time])
ylim([0,.11])
text(200-trans_time,.085,'$g_w(t)$','FontSize',10)
text(2600-trans_time,.07,'$g_r(t)$','FontSize',10)
text(-300-trans_time,.12,'c)')
subplot(311) 
effect_out_list = sqrt(2)*sqrt(1-cumtrapz(t,Eout.^2));
tau_list = cumtrapz(t,Dipolplt.^2/kappa);
tau_list_w = tau_list(find(t<trans_time-50));
tau_list_r = tau_list(find(t>= trans_time));
effect_out_list(find(t<trans_time)) = [];
effective_write(find(t>=trans_time-50)) = [];
length(effect_out_list)
length(tau_list_r)
length(effective_write)
length(tau_list_w)
chx2=plot(tau_list_w-3, effective_write.^2, tau_list_r-3, effect_out_list.^2)
%h12 = legend('Effective Write', 'Effective Read',1)
xlabel('$\tau$', 'fontsize',10)
%ylabel('$ns^{\frac{-1}{2}}$','fontsize',12)
xlim([-3,3])
ylim([0,2])
text(.6,.4,'$|\mathcal{E}_\textrm{in}(\tau)|^2$','FontSize',10)
text(4,.8,'$|\mathcal{E}_\textrm{out}(\tau)|^2$','FontSize',10)
text(-.5,2.6,'a)')
%laprint(190, '3plotsv8')
trapz(Eout.^2)
figure(19)
plot(tau_list_w, effective_write,tau_list_r, effect_out_list)
out = tau_list;
end
